    ...
    <>
      ...
      <section className="h-[100svh] flex items-center justify-center text-black">
        ...
        <div className="w-[640px] max-w-[90vw] h-full bg-[url('/images/mainbg.jpg')] bg-cover bg-center flex items-center justify-center">
          ...
          <div className="flex flex-col items-center justify-center text-center h-full py-[clamp(40px,8vh,96px)] w-full bg-[#ffffff50]">
            {/* 타이틀 */}
            ...
            <p className="w-[80%] max-w-[512px] tracking-[-0.02em] text-h2 font-medium wio-hero-subtitle">
            ...
            <button
              type="button"
              onClick={onStart}
              className="redButton group relative w-[50%] max-w-[350px] inline-flex items-center justify-center
                bg-brand-accent text-white text-findtit px-4 py-5 min-h-[30px] 
                transition-all duration-300 ease-out wio-hero-cta"
              aria-label="내 취향 찾기 모달 열기"
            >
              <span className="transition-opacity duration-200 group-hover:opacity-0">내 취향 찾기</span>

              <span className="absolute transition-opacity duration-200 opacity-0 group-hover:opacity-100">START</span>
            </button>
          </div>
        </div>
      </section>
